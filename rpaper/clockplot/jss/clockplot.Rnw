...
p1 + theme(axis.text = element_blank(), axis.ticks = element_blank()) + labs(x = "", y = "")
@
...
p1 + labs(title = "SMS Receiving Times")
@
...
p + labs(size = "Magnitude", title = "Earthquakes in Bangladesh since 2023")
@
...
week_chart(wtemp, high = "yellow") + labs(title = "Random Values by Day")
@
...
# Import data (100-row sample from Kaggle US Accidents dataset)
acdt <- read.csv("https://raw.githubusercontent.com/mahmudstat/open-analysis/main/data/usacc.csv")

# Basic clock chart - when do accidents happen?
p1 <- clock_chart(acdt, Time, Col = "darkred")
p1 + labs(title = "US Accidents: Time of Day")

# Multivariate: color by temperature, size by humidity
p2 <- clock_chart_qnt(acdt, time = Time, len = Humidity..., Col = Temperature.F.)
p2 + labs(size = "Humidity (%)", color = "Temperature (F)",
       title = "US Accidents: Time, Temperature & Humidity")

# Facet by weather condition
p3 <- clock_chart_qlt(acdt, time = Time, crit = Weather_Condition) +
  facet_wrap(~Weather_Condition) +
  labs(color = "Weather", title = "Accidents by Weather Condition")

# Cyclic view: average humidity by hour (ensure 24 hours)
humidity_by_hour <- acdt %>%
...
day_chart(hvalue = humidity_by_hour, high = "blue", low = "yellow") +
...
\pkg{clockplot}~\Sexpr{packageVersion("clockplot")} package,
\pkg{ggplot2}~\Sexpr{packageVersion("ggplot2")},
\pkg{dplyr}~\Sexpr{packageVersion("dplyr")}, and
