...
The history of statistical graphs and charts begins with William Playfair, who invented the bar chart in 1769, the line chart in 1786, and the pie chart in 1801 to illustrate economic data visually. Later, in 1833, John Herschel introduced the scatter plot to study astronomical observations, paving the way for analyzing relationships between variables. The histogram, created by Karl Pearson in 1895, became a cornerstone for visualizing frequency distributions, while John Tukey’s box plot (1970) revolutionized data summaries by highlighting medians, quartiles, and outliers. Geographic visualization emerged with Baron Pierre Charles Dupin’s choropleth map in 1826, representing regional statistics through color shading. In the late 20th century, heat maps gained traction in biology and finance for matrix and density visualization. Over time, modified versions—such as stacked bars, area charts, donut charts, violin plots, and interactive GIS maps—have expanded the interpretative power of these original designs, making them indispensable tools for modern statistical communication.
...
i. Construction of the 24-hour clock (the clock skeleton)
ii. Plotting arbitrary timestamped events on the clock

Each steps has two steps: the mathematical idea and the programming associated
...
It suffices to draw a unit circle, although a circle of any radius would do. 
In `R`, it is easiest to draw a circle using the complex equation. For a unit
circle, the expression is $e^{i\theta}$, where $e$ is Euler's number, an 
...
take values from 0 to 360 degrees ($2\pi$ in radian). The trigonometric form of the 
equation is $sin^2 \theta + cos^2 \theta = 1$, while the algebraic form is
$x^2+y^2 = 1$. The complex form, however, as we have mentioned, is more convenient. 

Programmatically, we need to create some angles ranging from 0 to $2\pi$ and 
then connect them to make a complete circle. Below we create 100 angles and use
them in the circle equation ($e^{i\theta}$). 

```{r, comc, fig.height=4, fig.width=4, fig.align='center', fig.cap="The Unit Circle"}
par(pty = "s")
k <- 100
timepoint <- exp(1i * 2 * pi * (k:1) / k)
plot(c(timepoint, timepoint[1]), type = "l",
     xlab = "", ylab = "")  # Add first point at end
```
...
First, we take a look at the output with 24 hour marks. 

```{r circle, fig.height=4, fig.width=4, fig.align='center', fig.cap="24 Hour Marks"}
par(pty = "s")
k <- 24
hour <- exp(1i * 2 * pi * (k:1) / k)
plot(hour, pch = 19, xlab = "", ylab = "")
```

Now, to implement the entire concept, we rely on the `R` package `ggplot2` to produce better graphics and achieve greater control. We need to create an `R` data frame for plotting ggplots. We also mark some points for minutes. 

First, we create the required data frame
...
, corresponding to the elapse of successive quarters (15 minutes). Thus, a small point on the Figure \@ref(fig:clock24) in the middle of two larger point (hour) means half an hour
has elapsed. For instance, the the point in the middle of 7 PM and 8 PM is 7:30 
PM. 

The code `exp(1i * 2 * pi * (k:1) / k)` gives us all the angles 
...
At 0 degree, we have 3 on a 12-hour clock and 6 on a 24-hours clock.

Similarly, the code `SubT = exp(1i * 2 * pi * (subk:1) / subk)` gives us the
angles corresponding to a quarter of an hour. As we have already mentioned, 
...
they are unequal dimensions. The points corresponding to the hours are labelled 
AM or PM using the `geom_text()` function. We do not need the axis labels and 
ticks. Let us remove them:

```{r clock24, fig.height=6, fig.width=6, fig.align='center', fig.cap="The Clock Skeleton"}
...
the last point. `aspect.ratio = 1` is set to ensure the chart size is not affected by the IDE. 
...
This completes our first part of the plot; we obtain the skeleton of the clock. 

The second and final part of the task involves plotting timestamped events on the
appropriate points. Now, this task has can be implemented in many ways. In the simplest 
